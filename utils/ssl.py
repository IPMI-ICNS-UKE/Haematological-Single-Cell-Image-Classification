...
from sparsam.utils import ModelMode, model_inference
...
def load_data_eval(data_set, batch_size=None):
    ...
    for split in splits:
        if data_set_config["split_root_path"] is not None:
            data_split_path = data_set_config["split_root_path"] / Path(
                split + "_split.csv"
            )
            try:
                ...
                print(
                    f'No file named {Path(split + "_split.csv")} in directory {data_set_config["split_root_path"]} '
                )
                data_split_path = (
                    current_path
                    / Path(f"splits/{data_set}")
                    / Path(split + "_split.csv")
                )
                images_data = pd.read_csv(data_split_path, index_col=False)
                print(f"{split}_split loaded from {data_split_path}")
        else:
            data_split_path = (
                current_path / Path(f"splits/{data_set}") / Path(split + "_split.csv")
            )
            images_data = pd.read_csv(data_split_path, index_col=False)
            print(f"{split}_split loaded from {data_split_path}")
...
def edit_used_images(image_paths, image_labels, seed=0, max_train_size=None):
    if max_train_size is not None:
        image_paths["train"], _, image_labels["train"], _ = train_test_split(
            image_paths["train"],
            image_labels["train"],
...
def create_dataset(data_set_config, image_paths, image_labels, data_augmentation=None):
    ...
    X_train, y_train, _, _ = uniform_train_test_splitting(
        X_train, y_train, n_samples_class=100
    )
    X_validation, y_validation, _, _ = uniform_train_test_splitting(
        X_validation, y_validation, n_samples_class=500
    )
    labeled_train_set = ImageSet(
        img_paths=X_train, labels=y_train, class_names=class_names, normalize=True
    )
    labeled_val_set = ImageSet(
        img_paths=X_validation,
        labels=y_validation,
        class_names=class_names,
        normalize=True,
    )
...
def extract_features(device, config, model, loader, splits=["train", "test"]):
    ...
    for split in splits:
        features[split], labels[split] = model_inference(
            loader[split], model=model, mode=ModelMode.EXTRACT_FEATURES, device=device
        )
...
def calculate_predictions(classifier_pipeline, features, labels):
    ...
    report = classification_report(
        labels["test"], preds, output_dict=True, zero_division=0
    )
    return report
...
def load_data_eval(data_set_fit, dataset_eval, batch_size=None):
    ...
    image_paths["train"], image_labels["train"] = load_paths(
        data_set_config_fit, "train", current_path, data_set_fit
    )
    image_paths["test"], image_labels["test"] = load_paths(
        data_set_config_eval, "test", current_path, dataset_eval
    )

    image_paths["train"], image_labels["train"], data_set_config_fit["class_names"] = (
        intersect_classes(
    ...
    image_paths["test"], image_labels["test"], data_set_config_eval["class_names"] = (
        intersect_classes(
...
def map_results(test_probas, labels_test, class_names_train, class_names_eval):
    ...
    train_index_to_string = {
        idx: class_name for idx, class_name in enumerate(class_names_train)
    }
    predicted = [train_index_to_string[idx.item()] for idx in predicted]
    if "MYC" in class_names_eval:
        predicted = [
            "MYC" if label in ["MMZ", "PMO", "MYB"] else label for label in predicted
        ]
    if "NGB" not in class_names_eval:
        predicted = ["NGS" if label in ["NGB"] else label for label in predicted]

    eval_index_to_class = {
        idx: class_name for idx, class_name in enumerate(class_names_eval)
    }
