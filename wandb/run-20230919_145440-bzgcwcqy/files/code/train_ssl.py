...
@click.option(
    ...
    type=click.BOOL, required=False,default=True,
)
@click.option(
    '--path_to_weights', type=click.STRING,required=False, default = '/home/lwenderoth/Documents/Doktorarbeit/Results/Weights/bm/pretrained_bm.pt',
...
def run(device,train,path_to_weights,dataset,batch_size,teacher_momentum,model,save_path,max_train_image_number,n_iter,
    ...
    report_path = evaluate(device, path_to_weights, dataset_fit, dataset_eval, batch_size, model, n_iter=n_iter,
                           save_dir=Path(config['save_path']) / 'evaluation')
    # update save parameter
    save_parameter = {'dataset': f'fit_{dataset_fit}_eval_{dataset_eval}', 'device': device, 'if train': train,
