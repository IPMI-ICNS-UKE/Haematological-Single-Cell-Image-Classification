...
def run(
    ...
    # Initialize WandB
    wandb.login()
    wandb.init(group='supervised', **config['wandb_parameter'], name=f'{project_name}')
    ...
    for iter in range(n_iter):
        ...
        if train:
            path_best_model_weights = train_supervised_model(
                ...
                model_name, max_train_image_number, num_epochs, class_names, batch_size, n_iter, resume
            )
        evaluate(
