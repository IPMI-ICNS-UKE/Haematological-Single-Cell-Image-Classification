...
def run(
        ...
        data_augmentation = torchvision.transforms.Compose(
            [
                torchvision.transforms.RandomApply(
                    [torchvision.transforms.Grayscale(num_output_channels=3)], p=0.05
                ),
    ...
    if max_train_image_number is not None:
        image_paths, image_labels = edit_used_images(
            image_paths, image_labels, seed=0, max_train_size=max_train_image_number
        )
    unlabeled_train_set, labeled_train_set, labeled_val_set, test_set = create_dataset(
        data_set_config, image_paths, image_labels, data_augmentation=data_augmentation
    )
    ...
    backbone = load_model(
        model=model, model_parameter=config["model_parameter"], device=device
    )
    ...
    save_parameter = {
        "dataset": f"fit_{dataset}_eval_{dataset}",
        "device": device,
        "if train": train,
        "path_to_weights": path_to_weights,
        "batch_size": batch_size,
