...
from sparsam.helper import recursive_dict, uniform_train_test_splitting
...
def summary_evaluation_results(report_path, n_sample, classifier):
    ...
    classifier_name = {
        "SVC": str(
            "SVC(C=1, break_ties=True, cache_size=10000, class_weight='balanced',\n    probability=True)"
        ),
    ...
    for sample in n_sample:
        ...
        for one_classifier in classifier:
            df_report_class = df_report_sample.loc[
                df_report_sample["classifier"] == classifier_name[one_classifier]
            ]
            ...
            df_results = pd.concat(
                [df_results, pd.DataFrame([new_row])], ignore_index=True
            )
...
def evaluate(
    ...
    config_fit, config_eval, train_loader, test_loader = load_data_eval(
        fit_dataset, eval_dataset, batch_size=batch_size
    )
    time = datetime.datetime.now()
    if save_dir is None:
        save_dir = (
            Path(config_fit["save_path"])
            / Path("Evaluation")
            / f"fit_{fit_dataset}_eval_{eval_dataset}"
            / Path(f"{time.year}_{time.month}_{time.day}-{time.hour}{time.minute}")
        )
    ...
    teacher = load_model(
        model=model, device=device, model_parameter=config_fit["model_parameter"]
    )
        ...
        for classifier_name in config_fit["classifier"]:
            ...
            for iteration in range(config_fit["n_iterations"]):
                train_features, train_labels, _, _ = uniform_train_test_splitting(
                    features["train"],
                    labels["train"],
                    n_samples_class=eval_class_size,
                    seed=int(iteration),
                )
                ...
                test_preds, eval_labels = map_results(
                    test_probas,
                    labels["test"],
                    config_fit["class_names"],
                    config_eval["class_names"],
                )

                report = classification_report(
                    eval_labels,
                    test_preds,
                    labels=config_fit["class_names"],
                    target_names=config_fit["class_names"],
                    output_dict=True,
                )
                results[eval_class_size][classifier.__class__.__name__][iteration][
                    "report"
                ] = report

                test_bal_acc = balanced_accuracy_score(eval_labels, test_preds)
                os.makedirs(
                    Path(config_fit["save_path"]) / "features_and_preds", exist_ok=True
                )
                with open(
                    Path(config_fit["save_path"])
                    / "features_and_preds"
                    / Path(
                        f"test_features_{classifier_name}_{eval_class_size}_iter_{iteration}.pt"
                    ),
                    "wb",
                ) as h:
                ...
                cm = confusion_matrix(
                    eval_labels, test_preds, labels=config_fit["class_names"]
                )
                cfms.append(np.array(cm))
                cm_df = pd.DataFrame(
                    cm,
                    index=config_fit["class_names"],
                    columns=config_fit["class_names"],
                )
                ...
                cm_df.to_csv(
                    ...
                    / "cmfs"
                    / Path(
                        f"confusion_matrix_{classifier_name}_{eval_class_size}_iter_{iteration}.csv"
                    ),
                    index=True,
                    header=True,
                )
                ...
                precision, recall, f1, _ = precision_recall_fscore_support(
                    eval_labels, test_preds, average=None
                )
                macro_precision = precision_score(
                    eval_labels, test_preds, average="macro"
                )
                ...
                metrics_df.to_csv(
                    ...
                    / "metrics"
                    / Path(
                        f"metrics_{classifier_name}_{eval_class_size}_iter_{iteration}.csv"
                    ),
                    index=True,
                    header=True,
                )
            ...
            cm_mean_df.to_csv(
                ...
                / "cmfs_overview"
                / Path(
                    f"confusion_matrix_{classifier_name}_{eval_class_size}_mean.csv"
                ),
                index=True,
                header=True,
            )
            cm_std_df = pd.DataFrame(
                std_array,
                index=config_fit["class_names"],
                columns=config_fit["class_names"],
            )
            cm_std_df.to_csv(
                Path(config_fit["save_path"])
                / "cmfs_overview"
                / Path(f"confusion_matrix_{classifier_name}_{eval_class_size}_std.csv"),
                index=True,
                header=True,
            )
            ...
            os.makedirs(
                Path(config_fit["save_path"]) / "metric_overview", exist_ok=True
            )
            mean_df.to_csv(
                Path(config_fit["save_path"])
                / "metric_overview"
                / Path(f"metric_{classifier_name}_{eval_class_size}_mean.csv"),
                index=True,
                header=True,
            )
            std_df.to_csv(
                Path(config_fit["save_path"])
                / "metric_overview"
                / Path(f"metric_{classifier_name}_{eval_class_size}_std.csv"),
                index=True,
                header=True,
            )
    ...
    results_df = pd.DataFrame(
        {key: pd.Series(value) for key, value in results_overview.items()}
    )
...
@click.option(
    "--batch_size", type=click.INT, required=True, default=4, help="Batchsize."
)
@click.option(
    "--teacher_momentum",
    type=click.FLOAT,
...
def run(
    ...
    save_path = (
        Path(save_path)
        / f"fit_{dataset_fit}_eval_{dataset_eval}"
        / Path(f"{time.year}_{time.month}_{time.day}-{time.hour}{time.minute}")
    )
    ...
    save_parameter = {
        "dataset": f"fit_{dataset_fit}_eval_{dataset_eval}",
        "device": device,
        "if train": train,
        ...
        "model": model,
        "save_path": save_path,
        "max_train_image_number": max_train_image_number,
        "n_iter_eval": n_iter,
    }
    ...
    summary_evaluation_results(
        report_path, n_sample=config["n_iterations"], classifier=classifier
    )


if __name__ == "__main__":
    warnings.filterwarnings(
        "ignore", category=UndefinedMetricWarning, module="sklearn.metrics"
    )
