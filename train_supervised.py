...
@click.option(
    "--batch_size", type=click.INT, required=True, default=4, help="Batch size."
)
@click.option(
    "--model_name",
    type=click.STRING,
...
def run(
    ...
    config, data_set_config, image_paths, image_labels, not_used_images = load_data(
        dataset
    )
    ...
    project_name = dataset / Path(
        f"{time.year}_{time.month}_{time.day}-{time.hour}{time.minute}"
    )

    # Initialize WandB
    wandb.login()
    wandb.init(
        group="supervised_test", **config["wandb_parameter"], name=f"{project_name}"
    )
...
def train_supervised_model(
    ...
    (
        ...
        image_paths["train"],
        image_labels["train"],
        stratify=image_labels["train"],
        test_size=0.3,
        random_state=42,
    )

    if max_train_image_number is not None:
        image_paths["train"], image_labels["train"], _, _ = (
            uniform_train_test_splitting(
                image_paths["train"],
                image_labels["train"],
                n_samples_class=max_train_image_number,
            )
        )
    ...
    val_set = ImageSet(
        img_paths=image_paths["val"],
        labels=image_labels["val"],
        class_names=class_names,
        normalize=True,
    )
    ...
    df_not_used_images.to_csv(
        Path(config["save_path"])
        / Path("image_used_info")
        / f"iter_{iter}_not_used_images.csv"
    )

    df_used_train_images = pd.DataFrame(image_paths["train"])
    df_used_train_images.to_csv(
        Path(config["save_path"])
        / Path("image_used_info")
        / f"iter_{iter}_used_train_images.csv"
    )

    # Initialize the model
    device = torch.device(device if torch.cuda.is_available() else "cpu")
    model = timm.create_model(
        model_name, pretrained=True, num_classes=len(class_names), in_chans=3
    )
    ...
    for epoch in range(num_epochs):
        ...
        with torch.no_grad():
            ...
            print(
                f"Epoch [{epoch + 1}/{num_epochs}], Balanced Accuracy: {balanced_acc}"
            )
        wandb.log(
            {
                "epoch": epoch + 1,
                "loss": loss.item(),
                "balanced_accuracy_val": balanced_acc,
            }
        )

        # Adjust learning rate using lr_scheduler
        lr_scheduler.step(balanced_acc)
        (save_path / "weights").mkdir(parents=True, exist_ok=True)
        torch.save(
            model.state_dict(),
            save_path / Path("weights") / Path(f"epoch_{epoch}_xcit_model.pt"),
        )
        ...
        if balanced_acc > best_balanced_acc:
            ...
            path_to_weights = (
                save_path / Path("weights") / Path(f"iter_{iter}_best_xcit_model.pt")
            )
