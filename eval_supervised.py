...
def evaluate(
    ...
    save_parameter = {
        ...
        "model": model_name,
        "save_path": save_path,
        "class_names_dataset": class_names_dataset,
    ...
    image_paths["test"], image_labels["test"] = map_classes(
        image_paths["test"], image_labels["test"], train_classes
    )
    test_set = ImageSet(
    ...
    model = timm.create_model(
        model_name, pretrained=True, num_classes=len(train_classes), in_chans=3
    )
    model.to(device)
    ...
    with torch.no_grad():
        for images, labels_eval in test_loader:
            ...
            mask_tensor = torch.tensor(
                [
                    1 if class_name in considered_train_class else 0
            ...
            train_index_to_string = {
                idx: class_name for idx, class_name in enumerate(train_classes)
            }
            predicted = [train_index_to_string[idx.item()] for idx in predicted]
            if "MYC" in eval_classes:
                predicted = [
                    "MYC" if label in ["MMZ", "PMO", "MYB"] else label
                    for label in predicted
                ]
            if "NGB" not in eval_classes:
                predicted = [
                    "NGS" if label in ["NGB"] else label for label in predicted
                ]

            labels_numpy = labels_eval.cpu().numpy()
            eval_index_to_class = {
                idx: class_name for idx, class_name in enumerate(eval_classes)
            }
    ...
    cm_df.to_csv(
        Path(config["save_path"]) / Path("confusion_matrix.csv"),
        index=True,
        header=True,
    )
    metrics_df.to_csv(
        Path(config["save_path"]) / Path("metrics.csv"), index=True, header=True
    )
